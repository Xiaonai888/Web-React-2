...
import { serializeStudioAdjustmentLayer, restoreStudioAdjustmentLayer } from './StudioAdjustmentLayerPersistence'
...
export function exportStudioLayerStack(stack) {
  ...
  return {
    ...
    layers: stack.layers.map((layer, index) => ({
      ...
      ...serializeStudioAdjustmentLayer(layer),
...
export async function loadStudioLayerStack(paper, displayCanvas) {
  ...
  const loaded = await Promise.all(paper.layers.map(async (item, index) => {
    ...
    const runtimeLayer = { id: item.id, name: item.name, canvas, visible: item.visible, locked: item.locked, opacity: item.opacity,
    ...
    if (runtimeLayer.isBackground && item.adjustment !== undefined) throw new Error('Background cannot be an adjustment layer.')
    return restoreStudioAdjustmentLayer(item, runtimeLayer)
  }))

...
export default function StoryBulkDownloadPage() {
  ...
  const selectable = episodes.filter((item) => !item.is_locked && !item.is_adult)
  const selectedIds = new Set(selected)

  function toggleEpisode(item) {
    if (working || item.is_adult) { if (item.is_adult) setMessage(t('storyBulkDownload.restricted')); return }
    const id = String(item.id)
    setSelected((current) => current.includes(id) ? current.filter((value) => value !== id) : [...current, id])
    setMessage(item.is_locked ? t('storyBulkDownload.locked') : '')
  }
  ...
  async function handleDownload() {
    ...
    try {
      for (const episodeId of selected) {
        ...
        try {
          await downloadOfflineEpisode({ accountId, storyId, episodeId, signal: controller.signal })
          completed += 1
        } catch {
          if (controller.signal.aborted) break
          failed += 1
      ...
      <div className="mx-auto flex min-h-[100dvh] max-w-3xl flex-col bg-[var(--shadow-bg-surface)]">
        ...
        <footer className="fixed bottom-0 left-0 right-0 z-20 mx-auto max-w-3xl border-t border-[var(--shadow-border)] bg-[var(--shadow-bg-surface)] px-4 pb-[max(16px,env(safe-area-inset-bottom))] pt-3 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            <button type="button" disabled={working || loading || loadError || !selectable.length} onClick={() => { setSelected(selected.length === selectable.length && selectable.every((item) => selectedIds.has(String(item.id))) ? [] : selectable.map((item) => String(item.id))); setMessage('') }} className="min-h-12 shrink-0 rounded-full border border-[var(--shadow-border)] px-4 text-[12px] font-semibold disabled:opacity-50">{selected.length === selectable.length && selectable.every((item) => selectedIds.has(String(item.id))) ? t('storyBulkDownload.clear') : t('storyBulkDownload.selectAll')}</button>
            <button type="button" disabled={working || !selected.length} onClick={handleDownload} className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-[#eb315b] px-4 text-[14px] font-bold text-white disabled:opacity-50"><i className="fa-solid fa-download" />{working ? t('storyBulkDownload.downloading', { done: progress, total: selected.length }) : `${t('storyBulkDownload.download')} (${selected.length})`}</button>
          </div>
        </footer>
      </div>
    </div>

  ...
  async function loadProducts(options = {}) {
    ...
    try {
      ...
      if (!response.ok || data.ok === false) {
        throw new Error(data.message || 'Failed to load Second Hand books')
      }
      ...
      setProducts([])
      setMessage(error.message || 'Failed to load Second Hand books')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#f5f3fa] pb-[110px]">
      <header className="sticky top-0 z-50 bg-white/95 px-4 py-3 shadow-sm backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="line-clamp-1 text-[18px] font-extrabold text-[#111827]">Second Hand</h1>
            <p className="mt-0.5 line-clamp-1 text-[11.5px] font-semibold text-[#8d94a1]">
              {meta.total} books · Checked condition, lower price, limited stock.
            </p>
          </div>
          ...
          <button
            ...
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#f5f3fa] text-[#111827] active:scale-95"
            aria-label="Search Second Hand books"
          >
      ...
      <main className="mx-auto max-w-6xl px-4 pt-4">
        <section className="mb-3 overflow-hidden rounded-[24px] bg-[#eef2ff] px-4 py-4 shadow-sm ring-1 ring-[#dbe4ff]/80">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[18px] bg-white/85 text-[#4f46e5]">
              <i className="fa-solid fa-recycle text-[18px]" />
            </div>

            <div className="min-w-0 flex-1">
              <h2 className="text-[16px] font-extrabold text-[#111827]">
                Checked second hand books
              </h2>
              <p className="mt-1 text-[12px] font-semibold leading-5 text-[#4f46e5]/80">
                Lower price books with limited stock. Check condition and price before ordering.
              </p>
            </div>
          </div>
        </section>
          ...
          <form onSubmit={handleSearchSubmit} className="rounded-[22px] bg-white p-3 shadow-sm ring-1 ring-black/5">
            <div className="flex items-center gap-2 rounded-full bg-[#f4f5f7] px-4 py-3">
              ...
              <input
                ...
                placeholder="Search second hand books or authors"
        ...
        {loading && !products.length ? (
          ...
          <section className="mt-4 rounded-[26px] bg-white px-5 py-12 text-center shadow-sm ring-1 ring-black/5">
            ...
            <h2 className="mt-4 text-[18px] font-extrabold text-[#111827]">No second hand books found</h2>
            <p className="mt-2 text-[13px] leading-6 text-[#8d94a1]">
              Try another search or check Second Hand again later.
            </p>
            <button
              type="button"
              onClick={() => navigate('/shop')}

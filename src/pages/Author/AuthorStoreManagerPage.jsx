...
function EmptyState({ title, text }) {
  return (
    <div className="rounded-[28px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
      ...
      <h3 className="text-[17px] font-black text-[#111827]">{title}</h3>
  ...
  return (
    <div className="overflow-hidden rounded-[24px] bg-white shadow-sm ring-1 ring-black/5">
      <div className="relative aspect-[3/4] bg-[#f3f4f6]">
        ...
        <div className="absolute left-2 top-2 rounded-full bg-white/95 px-2.5 py-1 text-[10px] font-black text-[#111827] shadow-sm">{product.type}</div>
        <button type="button" className="absolute bottom-2 right-2 flex h-10 w-10 items-center justify-center rounded-full bg-[#111827] text-white shadow-lg">
      ...
      <div className="p-3">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          ...
          <span className="rounded-full bg-[#f5f3fa] px-2 py-1 text-[10px] font-black text-[#111827]">{product.status}</span>
        </div>
        ...
        <div className="mt-1 text-[11px] font-bold text-[#8b93a1]">
          {product.type === 'Book' ? `${product.stock || 0} stock • ${product.condition}${product.condition === 'Second Hand' && product.qualityPercent ? ` • ${product.qualityPercent}%` : ''}` : `${product.pageCount || 0} pages • PDF`}
        </div>
      </div>
    </div>
  )
...
function StoreManagerHome({ activeTab, setActiveTab, activeType, setActiveType, filteredProducts, categories, newCategory, setNewCategory, addCategory, onAddProduct }) {
  return (
      ...
      <section className="rounded-[28px] bg-white p-4 shadow-sm ring-1 ring-black/5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[12px] font-semibold leading-5 text-[#8b93a1]">Manage products, categories, and store orders.</p>
          </div>
          <button type="button" onClick={onAddProduct} className="hidden h-10 rounded-full bg-[#111827] px-4 text-[12px] font-black text-white sm:block">Add Product</button>
        </div>
      ...
      {activeTab === 'Products' ? (
        <section className="mt-4 space-y-4">
          ...
          {filteredProducts.length ? (
            ...
            <EmptyState title="No products yet" text="Add a paper book or PDF product. Backend saving and real checkout will be connected later." />
          )}
        </section>
      ) : null}
        ...
        <section className="mt-4 space-y-3">
          <div className="rounded-[24px] bg-white p-4 shadow-sm ring-1 ring-black/5">
            ...
            <div className="mt-3 flex gap-2">
              ...
              <button type="button" onClick={addCategory} className="h-11 shrink-0 rounded-2xl bg-[#111827] px-4 text-[12px] font-black text-white">Add</button>
            </div>
          </div>
  ...
  return (
    <main className="mx-auto max-w-[980px] px-4 py-4">
      <section className="rounded-[28px] bg-white p-4 shadow-sm ring-1 ring-black/5">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 rounded-2xl bg-[#f3f4f6] p-1">
          <button type="button" onClick={() => setType('Book')} className={`h-11 rounded-xl text-[13px] font-black ${type === 'Book' ? 'bg-[#111827] text-white' : 'text-[#6b7280]'}`}>Book</button>
          <button type="button" onClick={() => setType('PDF')} className={`h-11 rounded-xl text-[13px] font-black ${type === 'PDF' ? 'bg-[#111827] text-white' : 'text-[#6b7280]'}`}>PDF</button>
        </div>
      </section>

      <section className="mt-4 rounded-[28px] bg-white p-4 shadow-sm ring-1 ring-black/5">
        <h2 className="text-[16px] font-black text-[#111827]">Product cover</h2>
        <p className="mt-1 text-[12px] font-semibold leading-5 text-[#8b93a1]">Upload 1 vertical cover image. Real storage upload will be connected later.</p>

        <div className="mt-4 flex gap-4">
          <div className="aspect-[3/4] w-[116px] shrink-0 overflow-hidden rounded-[20px] bg-[#f3f4f6] ring-1 ring-black/5">
            {coverPreview ? (
          ...
          <div className="min-w-0 flex-1">
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} className="h-11 w-full rounded-2xl bg-[#111827] text-[12px] font-black text-white">Upload Cover</button>
            {coverPreview ? <button type="button" onClick={removeCover} className="mt-2 h-11 w-full rounded-2xl border border-[#e5e7eb] bg-white text-[12px] font-black text-[#111827]">Remove</button> : null}
            {coverFileName ? <p className="mt-2 truncate text-[11px] font-bold text-[#8b93a1]">{coverFileName}</p> : null}
          </div>
        </div>
      </section>
      ...
      <section className="mt-4 space-y-4 rounded-[28px] bg-white p-4 shadow-sm ring-1 ring-black/5">
        ...
        <div>
          ...
          <textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Short product description" className="min-h-[110px] w-full rounded-2xl border border-[#e5e7eb] bg-white px-3.5 py-3 text-[13px] font-bold text-[#111827] outline-none focus:border-[#111827]" />
        </div>
      </section>
      ...
      <div className="sticky bottom-[76px] mt-4 grid grid-cols-2 gap-3 rounded-[24px] border border-[#eef0f4] bg-white/95 p-3 shadow-xl backdrop-blur">
        <button type="button" onClick={onBack} className="h-12 rounded-full border border-[#e5e7eb] bg-white text-[13px] font-black text-[#111827]">Cancel</button>
    ...
    <div className="min-h-screen bg-[#f3f4f6] pb-[92px]">
      <div className="sticky top-0 z-40 border-b border-[#eef0f4] bg-white/95 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-[980px] items-center justify-between px-4">
          <button type="button" onClick={() => (mode === 'form' ? setMode('manager') : navigate('/author/page'))} className="flex h-10 w-10 items-center justify-center rounded-full text-[#111827] active:bg-[#f3f4f6]">
          ...
          <button type="button" onClick={() => setMode('form')} className={`flex h-10 w-10 items-center justify-center rounded-full bg-[#111827] text-white ${mode === 'form' ? 'invisible' : ''}`}>

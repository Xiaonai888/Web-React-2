...
registerTranslationNamespace('authorStoreWithdrawalDetails', {
  en: { title: 'Withdrawal Details', available: 'Available for Withdrawal', minimum: 'Minimum withdrawal: $10.00', request: 'Request Withdrawal', all: 'All', book: 'Book', pdf: 'PDF', history: 'Earnings history', empty: 'No earnings to display yet.', pending: 'Loading verified balance...', failed: 'Could not load your balance. Please try again.', price: 'Product price', shipping: 'Delivery fee', company: 'Delivery company', fee: 'Service fee', amount: 'Amount credited', order: 'Order', loadMore: 'Load more', loadingEarnings: 'Loading earnings...', earningsFailed: 'Could not load earnings. Please retry.', retry: 'Retry', back: 'Back' },
  km: { title: 'ព័ត៌មានលម្អិតប្រាក់ដក', available: 'ប្រាក់ដែលអាចដកបាន', minimum: 'ចំនួនដកអប្បបរមា៖ $10.00', request: 'ស្នើសុំដកប្រាក់', all: 'ទាំងអស់', book: 'សៀវភៅ', pdf: 'PDF', history: 'ប្រវត្តិចំណូល', empty: 'មិនទាន់មានចំណូលត្រូវបង្ហាញ', pending: 'កំពុងផ្ទុកសមតុល្យប្រាក់ដែលបានផ្ទៀងផ្ទាត់...', failed: 'មិនអាចផ្ទុកសមតុល្យប្រាក់បានទេ។ សូមសាកល្បងម្តងទៀត។', price: 'តម្លៃផលិតផល', shipping: 'ថ្លៃដឹកជញ្ជូន', company: 'ក្រុមហ៊ុនដឹកជញ្ជូន', fee: 'ថ្លៃសេវា', amount: 'ប្រាក់ដែលបានបញ្ចូល', order: 'ការបញ្ជាទិញ', loadMore: 'មើលបន្ថែម', loadingEarnings: 'កំពុងផ្ទុកប្រវត្តិចំណូល...', earningsFailed: 'មិនអាចផ្ទុកប្រវត្តិចំណូលបានទេ។ សូមសាកល្បងម្តងទៀត។', retry: 'សាកល្បងម្តងទៀត', back: 'ត្រឡប់ក្រោយ' },
  zh: { title: '提现详情', available: '可提现金额', minimum: '最低提现金额：$10.00', request: '申请提现', all: '全部', book: '纸质书', pdf: 'PDF', history: '收入记录', empty: '暂无收入记录', pending: '正在加载已核实的余额...', failed: '无法加载余额，请重试。', price: '商品价格', shipping: '配送费', company: '配送公司', fee: '服务费', amount: '入账金额', order: '订单', loadMore: '加载更多', loadingEarnings: '正在加载收入记录...', earningsFailed: '无法加载收入记录，请重试。', retry: '重试', back: '返回' },
  ja: { title: '出金明細', available: '出金可能額', minimum: '最低出金額：$10.00', request: '出金を申請', all: 'すべて', book: '書籍', pdf: 'PDF', history: '収益履歴', empty: '収益履歴はありません', pending: '確認済みの残高を読み込み中...', failed: '残高を読み込めませんでした。もう一度お試しください。', price: '商品価格', shipping: '配送料', company: '配送会社', fee: 'サービス手数料', amount: '計上額', order: '注文', loadMore: 'さらに表示', loadingEarnings: '収益履歴を読み込み中...', earningsFailed: '収益履歴を読み込めませんでした。もう一度お試しください。', retry: '再試行', back: '戻る' },
  ko: { title: '출금 상세', available: '출금 가능 금액', minimum: '최소 출금액: $10.00', request: '출금 신청', all: '전체', book: '책', pdf: 'PDF', history: '수익 내역', empty: '표시할 수익이 없습니다', pending: '확인된 잔액을 불러오는 중...', failed: '잔액을 불러오지 못했습니다. 다시 시도해 주세요.', price: '상품 가격', shipping: '배송비', company: '배송 업체', fee: '서비스 수수료', amount: '적립 금액', order: '주문', loadMore: '더 보기', loadingEarnings: '수익 내역을 불러오는 중...', earningsFailed: '수익 내역을 불러오지 못했습니다. 다시 시도해 주세요.', retry: '다시 시도', back: '뒤로' },
})
...
export default function AuthorStoreWithdrawalDetailsPage() {
  ...
  useEffect(() => {
    ...
    async function loadEarnings() {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/author-store/me/earnings?page=${earningsPage}&limit=20`, {
          headers: { Authorization: `Bearer ${token}` },
          signal: controller.signal,
        })
        const data = await response.json()
        if (!response.ok || data.ok === false || !Array.isArray(data.earnings)) {
          throw new Error('Invalid earnings data')
        }
        if (controller.signal.aborted) return
        setEarnings((previous) => earningsPage === 1 ? data.earnings : [...previous, ...data.earnings])
        setHasMoreEarnings(data.has_next === true)
      } catch {
        if (!controller.signal.aborted) setEarningsError(true)
      } finally {
        if (!controller.signal.aborted) setLoadingEarnings(false)
      }
    }

    loadEarnings()
    return () => controller.abort()
  }, [earningsPage, earningsRetry])

  const translate = (key) => t(`authorStoreWithdrawalDetails.${key}`)
  const list = useMemo(() => earnings.filter((item) => (filter === 'all' || item.type === filter) && (item.type === 'book' || item.type === 'pdf')), [earnings, filter])
    ...
    <main className="min-h-screen bg-[var(--shadow-bg-soft)] px-4 pb-24 pt-4 text-[var(--shadow-text-primary)]">
      <div className="mx-auto max-w-xl">
        ...
        <section className="mt-5">
          ...
          {list.length === 0 && !loadingEarnings && !earningsError && <div className="rounded-2xl bg-[var(--shadow-bg-surface)] p-8 text-center text-sm text-[var(--shadow-text-secondary)] ring-1 ring-[var(--shadow-border)]">{translate('empty')}</div>}
          ...
          {loadingEarnings && <p role="status" className="mt-3 text-center text-xs text-[var(--shadow-text-secondary)]">{translate('loadingEarnings')}</p>}
          {earningsError && <div role="alert" className="mt-3 flex flex-wrap items-center justify-center gap-2 text-xs text-red-600"><span>{translate('earningsFailed')}</span><button type="button" className="rounded-full border border-[var(--shadow-border)] px-3 py-2 font-bold" onClick={() => setEarningsRetry((value) => value + 1)}>{translate('retry')}</button></div>}
          {!loadingEarnings && !earningsError && hasMoreEarnings && <button type="button" onClick={() => setEarningsPage((page) => page + 1)} className="mt-4 w-full rounded-2xl border border-[var(--shadow-border)] bg-[var(--shadow-bg-surface)] px-4 py-3 text-sm font-bold">{translate('loadMore')}</button>}
        </section>
      </div>
    </main>
  )

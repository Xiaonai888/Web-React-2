...
const filters = [
  { label: 'All Hidden Identity', value: 'all' },
...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'Hidden Identity',
...
function isHiddenIdentityStory(item) {
  const values = [
    item.genre,
    item.category,
    item.main_genre,
  ...
  return values.some((value) => {
    ...
    return text.includes('hidden identity') || text.includes('hidden-identity') || text.includes('secret identity')
  })
}
...
function isSecretStory(story) {
  ...
  return ['hidden identity', 'secret identity', 'disguise', 'mask', 'double life', 'hidden truth', 'unknown past', 'reveal', 'secret', 'identity', 'fake name'].some((word) =>
    text.includes(word)
  )
}
...
function StoryCard({ story, onOpen }) {
  return (
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#ccfbf1]">
        {story.cover ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[#0f766e]">
            <i className="fa-solid fa-mask text-[28px]" />
          </div>
        )}
      </div>
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A hidden identity story about secrets, disguise, double life, unknown past, and dramatic reveals.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#ccfbf1] px-2.5 py-1 text-[#0f766e]">Hidden Identity</span>
...
export default function HiddenIdentityGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadHiddenIdentityStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=Hidden%20Identity&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load hidden identity stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isHiddenIdentityStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load hidden identity stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'secret') {
      return stories.filter(isSecretStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All Hidden Identity'
  ...
  return (
    <div className="min-h-screen bg-[#f7fbfb] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#ccfbf1] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-[#ccfbf1] text-[#111827] active:scale-95"
            aria-label="Back"
          >
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">Hidden Identity</h1>
            <p className="text-[11px] font-bold text-[#0f766e]">Secrets, disguise, and hidden truth</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#0f172a] via-[#0f766e] to-[#cbd5e1] p-5 text-white shadow-sm ring-1 ring-[#ccfbf1]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#0f766e] shadow-sm">
              <i className="fa-solid fa-mask text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#99f6e4]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8">Hidden Identity</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-white/85">
                Stories about secret names, disguise, double lives, unknown pasts, hidden truth, and dramatic reveals.
              </p>
            </div>
          </div>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            <div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-4 border-[#ccfbf1] border-t-[#111827]" />
            <div className="text-[13px] font-black text-[#667085]">Opening hidden identity...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest Hidden Identity" subtitle="Recently updated hidden identity stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular Hidden Identity" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#ccfbf1] text-[#0f766e]">
                    <i className="fa-solid fa-mask text-[24px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No hidden identity stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">Hidden identity stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'CEO',
...
function isCEOStory(item) {
  ...
  return values.some((value) => String(value || '').toLowerCase().includes('ceo'))
}
...
function isLuxuryStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['ceo', 'boss', 'billionaire', 'rich', 'luxury', 'office', 'company', 'contract', 'business', 'cold', 'president', 'secretary', 'marriage'].some((word) =>
    text.includes(word)
  )
}
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#fef3c7]">
        {story.cover ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[#b45309]">
            <i className="fa-solid fa-briefcase text-[28px]" />
          </div>
        )}
      </div>
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A CEO story about luxury romance, business power, cold bosses, contracts, secrets, and rich life.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#fef3c7] px-2.5 py-1 text-[#b45309]">CEO</span>
...
export default function CEOGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadCEOStories() {
      ...
      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=CEO&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load CEO stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isCEOStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load CEO stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'luxury') {
      return stories.filter(isLuxuryStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All CEO'
  ...
  return (
    <div className="min-h-screen bg-[#fffaf0] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#fde68a] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">CEO</h1>
            <p className="text-[11px] font-bold text-[#b45309]">Luxury romance, power, and business secrets</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#050505] via-[#78350f] to-[#facc15] p-5 text-white shadow-sm ring-1 ring-[#fde68a]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#b45309] shadow-sm">
              <i className="fa-solid fa-briefcase text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#fde68a]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8">CEO</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-white/85">
                Stories about powerful CEOs, luxury romance, cold bosses, contracts, office secrets, wealth, and business love.
              </p>
            </div>
          </div>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            ...
            <div className="text-[13px] font-black text-[#667085]">Opening CEO...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest CEO" subtitle="Recently updated CEO stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular CEO" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#fef3c7] text-[#b45309]">
                    <i className="fa-solid fa-briefcase text-[24px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No CEO stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">CEO stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

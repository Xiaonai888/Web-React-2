...
function isCompletedStory(story) {
  return Boolean(story?.is_completed) || String(story?.story_status || '').trim().toLowerCase() === 'completed'
}
...
export default function RomanceCompletedPage() {
  ...
  async function loadStories() {
    try {
      ...
      const response = await fetch(`${API_URL}/api/public/stories?genre=Romance&limit=100&sort=updated`)
      const data = await response.json().catch(() => ({}))

      if (!response.ok || data.ok === false) {
        throw new Error(data.message || 'Failed to load completed romance stories')
      }

      setStories(
        (data.stories || [])
          .filter((story) => isRomanceStory(story) && isCompletedStory(story))
          .map(normalizeStory)
      )
    } catch (error) {
      setStories([])
      setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load completed romance stories')
    } finally {
      setLoading(false)
    }
  }
  ...
  const completedStories = useMemo(
    () => [...stories].sort((a, b) => getTime(b.updatedAt) - getTime(a.updatedAt)),
    [stories]
  )
    ...
    <div className="min-h-screen bg-white pb-8">
      <header className="sticky top-0 z-40 border-b border-gray-100 bg-white shadow-sm">
        <div className="mx-auto flex h-[58px] max-w-5xl items-center px-4">
          ...
          <h1 className="ml-1 text-[20px] font-[650] text-[#111827]">Romance Completed</h1>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl">
        ...
        {!loading && !message && completedStories.length === 0 ? (
          <div className="px-5 py-16 text-center text-[14px] text-gray-400">No completed romance stories yet.</div>
        ) : null}
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-5 px-4 pt-5 md:grid-cols-4 lg:grid-cols-6">
            {completedStories.map((story) => (
              <button
                key={story.id}
                type="button"
                onClick={() => navigate(`/story/${story.id}`, { state: { returnTo: '/genre/romance/completed' } })}
              ...
              >
                ...
                <p className="mt-2 text-[12px] font-normal text-gray-400">
                  {story.totalEpisodes} {story.totalEpisodes === 1 ? 'Episode' : 'Episodes'}
                </p>
              </button>
            ))}
          </div>

...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'Thriller',
...
function isThrillerStory(item) {
  ...
  return values.some((value) => String(value || '').toLowerCase().includes('thriller'))
}
...
function isSuspenseStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['thriller', 'suspense', 'danger', 'chase', 'secret', 'psychological', 'escape', 'killer', 'fear', 'crime', 'trap', 'warning', 'tension'].some((word) =>
    text.includes(word)
  )
}
...
function StoryCard({ story, onOpen }) {
  return (
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#fff7ed]">
        {story.cover ? (
          <img src={story.cover} alt={story.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
        ) : (
          <div className="flex h-full w-full items-center justify-center text-[#ea580c]">
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A thriller story full of suspense, danger, secrets, psychological tension, and fast-moving fear.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#ffedd5] px-2.5 py-1 text-[#ea580c]">Thriller</span>
...
export default function ThrillerGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadThrillerStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=Thriller&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load thriller stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isThrillerStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load thriller stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'suspense') {
      return stories.filter(isSuspenseStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All Thriller'
  ...
  return (
    <div className="min-h-screen bg-[#fff7ed] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#fed7aa] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">Thriller</h1>
            <p className="text-[11px] font-bold text-[#ea580c]">Danger, suspense, and dark secrets</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#111827] via-[#7c2d12] to-[#ea580c] p-5 text-white shadow-sm ring-1 ring-[#fed7aa]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#ea580c] shadow-sm">
              <i className="fa-solid fa-triangle-exclamation text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#fed7aa]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8">Thriller</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-white/85">
                Stories with suspense, danger, chase, dark secrets, psychological pressure, and tension that keeps readers turning pages.
              </p>
            </div>
          </div>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            ...
            <div className="text-[13px] font-black text-[#667085]">Opening thriller...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest Thriller" subtitle="Recently updated thriller stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular Thriller" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
            ...
            <section className="mt-7">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#ffedd5] text-[#ea580c]">
                    <i className="fa-solid fa-triangle-exclamation text-[24px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No thriller stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">Thriller stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'Comedy',
...
function isComedyStory(item) {
  ...
  return values.some((value) => String(value || '').toLowerCase().includes('comedy'))
}
...
function isFunnyStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['funny', 'laugh', 'comedy', 'humor', 'joke', 'chaos', 'silly', 'cute', 'happy', 'playful', 'light'].some((word) =>
    text.includes(word)
  )
}
...
function StoryCard({ story, onOpen }) {
  return (
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#fff7d6]">
        {story.cover ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[#f59e0b]">
            <i className="fa-solid fa-face-laugh text-[28px]" />
          </div>
        )}
      </div>
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A light comedy story full of fun, laughter, playful moments, and relaxing energy.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#fff7d6] px-2.5 py-1 text-[#d97706]">Comedy</span>
...
export default function ComedyGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadComedyStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=Comedy&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load comedy stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isComedyStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load comedy stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'funny') {
      return stories.filter(isFunnyStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All Comedy'
  ...
  return (
    <div className="min-h-screen bg-[#fffaf0] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#fde68a] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-[#fff7d6] text-[#111827] active:scale-95"
            aria-label="Back"
          >
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">Comedy</h1>
            <p className="text-[11px] font-bold text-[#d97706]">Fun, laughter, and light moments</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#fff7d6] via-white to-[#fed7aa] p-5 shadow-sm ring-1 ring-[#fde68a]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#f59e0b] shadow-sm">
              <i className="fa-solid fa-face-laugh text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#d97706]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8 text-[#111827]">Comedy</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-[#667085]">
                Light, funny, playful stories made for readers who want smiles, chaos, and relaxing moments.
              </p>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-[12px] font-black active:scale-95 ${
                    active ? 'bg-[#111827] text-white' : 'bg-white text-[#d97706] ring-1 ring-[#fde68a]'
                  }`}
                >
                  {item.label}
                </button>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            ...
            <div className="text-[13px] font-black text-[#667085]">Opening comedy...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest Comedy" subtitle="Recently updated comedy stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular Comedy" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
            ...
            <section className="mt-7">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#fff7d6] text-[#f59e0b]">
                    <i className="fa-solid fa-face-laugh text-[24px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No comedy stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">Comedy stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

...
export default function RomanceUpdatesPage() {
  ...
  async function loadStories() {
    try {
      ...
      if (!response.ok || data.ok === false) {
        throw new Error(data.message || 'Failed to load romance updates')
      }

      setStories((data.stories || []).filter(isRomanceStory).map(normalizeStory))
    } catch (error) {
      setStories([])
      setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load romance updates')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-white pb-8">
      <header className="sticky top-0 z-40 border-b border-gray-100 bg-white shadow-sm">
        <div className="mx-auto flex h-[58px] max-w-5xl items-center px-4">
          ...
          <h1 className="ml-1 text-[20px] font-[650] text-[#111827]">Romance Updates</h1>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl">
        ...
        {!loading && !message && updatedStories.length === 0 ? (
          <div className="px-5 py-16 text-center text-[14px] text-gray-400">No romance updates yet.</div>
        ) : null}
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-5 px-4 pt-5 md:grid-cols-4 lg:grid-cols-6">
            {updatedStories.map((story) => (
              <button
                key={story.id}
                type="button"
                onClick={() => navigate(`/story/${story.id}`, { state: { returnTo: '/genre/romance/updates' } })}

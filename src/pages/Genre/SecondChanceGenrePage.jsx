...
const filters = [
  { label: 'All Second Chance', value: 'all' },
...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'Second Chance',
...
function isSecondChanceStory(item) {
  const values = [
    item.genre,
    item.category,
    item.main_genre,
  ...
  return values.some((value) => {
    ...
    return text.includes('second chance') || text.includes('second-chance') || text.includes('secondchance')
  })
}
...
function isRestartStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['second chance', 'restart', 'again', 'return', 'regret', 'healing', 'forgiveness', 'new beginning', 'love again', 'reunite', 'start over'].some((word) =>
    text.includes(word)
  )
}
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#dbeafe]">
        {story.cover ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[#2563eb]">
            <i className="fa-solid fa-rotate-left text-[28px]" />
          </div>
        )}
      </div>
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A second chance story about regret, healing, returning, forgiveness, new beginnings, and love again.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#dbeafe] px-2.5 py-1 text-[#2563eb]">Second Chance</span>
...
export default function SecondChanceGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadSecondChanceStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=Second%20Chance&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load second chance stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isSecondChanceStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load second chance stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'restart') {
      return stories.filter(isRestartStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All Second Chance'
  ...
  return (
    <div className="min-h-screen bg-[#f7fbff] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#bfdbfe] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">Second Chance</h1>
            <p className="text-[11px] font-bold text-[#2563eb]">Regret, healing, restart, and love again</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#2563eb] via-[#f7fbff] to-[#facc15] p-5 shadow-sm ring-1 ring-[#bfdbfe]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#2563eb] shadow-sm">
              <i className="fa-solid fa-rotate-left text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#2563eb]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8 text-[#111827]">Second Chance</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-[#667085]">
                Stories about regret, returning, healing, forgiveness, new beginnings, second love, and starting over again.
              </p>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-[12px] font-black active:scale-95 ${
                    active ? 'bg-[#111827] text-white' : 'bg-white text-[#2563eb] ring-1 ring-[#bfdbfe]'
                  }`}
                >
                  {item.label}
                </button>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            ...
            <div className="text-[13px] font-black text-[#667085]">Opening second chance...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest Second Chance" subtitle="Recently updated second chance stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular Second Chance" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
            ...
            <section className="mt-7">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  ...
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No second chance stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">Second chance stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'LGBTQ+',
...
function isLGBTQStory(item) {
  ...
  return values.some((value) => {
    ...
    return text.includes('lgbtq') || text.includes('lgbt') || text.includes('gl') || text.includes('bl') || text.includes('girls love') || text.includes('boys love')
  })
}
...
function isPrideStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['pride', 'identity', 'love', 'freedom', 'acceptance', 'girls love', 'boys love', 'lgbtq', 'lgbt', 'gl', 'bl', 'heart'].some((word) =>
    text.includes(word)
  )
}
...
function StoryCard({ story, onOpen }) {
  return (
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#f3e8ff]">
        {story.cover ? (
          <img src={story.cover} alt={story.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
        ) : (
          <div className="flex h-full w-full items-center justify-center text-[#9333ea]">
            <i className="fa-solid fa-rainbow text-[28px]" />
          </div>
        )}
      </div>
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'An LGBTQ+ story about love, identity, acceptance, emotion, and the freedom to be yourself.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#f3e8ff] px-2.5 py-1 text-[#9333ea]">LGBTQ+</span>
...
export default function LGBTQGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadLGBTQStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=LGBTQ%2B&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load LGBTQ+ stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isLGBTQStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load LGBTQ+ stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'pride') {
      return stories.filter(isPrideStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All LGBTQ+'
  ...
  return (
    <div className="min-h-screen bg-[#fbf7ff] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#e9d5ff] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f3e8ff] text-[#111827] active:scale-95"
            aria-label="Back"
          >
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">LGBTQ+</h1>
            <p className="text-[11px] font-bold text-[#9333ea]">Love, identity, and emotional freedom</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#f3e8ff] via-white to-[#bae6fd] p-5 shadow-sm ring-1 ring-[#e9d5ff]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#9333ea] shadow-sm">
              <i className="fa-solid fa-rainbow text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#9333ea]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8 text-[#111827]">LGBTQ+</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-[#667085]">
                Stories about love, identity, acceptance, courage, emotional freedom, and choosing your own heart.
              </p>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-[12px] font-black active:scale-95 ${
                    active ? 'bg-[#111827] text-white' : 'bg-white text-[#9333ea] ring-1 ring-[#e9d5ff]'
                  }`}
                >
                  {item.label}
                </button>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            <div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-4 border-[#e9d5ff] border-t-[#111827]" />
            <div className="text-[13px] font-black text-[#667085]">Opening LGBTQ+...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest LGBTQ+" subtitle="Recently updated LGBTQ+ stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular LGBTQ+" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#f3e8ff] text-[#9333ea]">
                    <i className="fa-solid fa-rainbow text-[24px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No LGBTQ+ stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">LGBTQ+ stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

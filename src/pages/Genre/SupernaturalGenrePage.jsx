...
const filters = [
  { label: 'All Supernatural', value: 'all' },
...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'Supernatural',
...
function isSupernaturalStory(item) {
  const values = [
    item.genre,
    item.category,
    item.main_genre,
  ...
  return values.some((value) => String(value || '').toLowerCase().includes('supernatural'))
}
...
function isParanormalStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['supernatural', 'paranormal', 'spirit', 'curse', 'magic', 'power', 'moon', 'ghost', 'hidden world', 'psychic', 'demon', 'angel', 'soul', 'unseen'].some((word) =>
    text.includes(word)
  )
}
    ...
    >
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A supernatural story about hidden powers, spirits, magic energy, curses, and unseen worlds.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#ede9fe] px-2.5 py-1 text-[#7c3aed]">Supernatural</span>
...
export default function SupernaturalGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadSupernaturalStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=Supernatural&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load supernatural stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isSupernaturalStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load supernatural stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'paranormal') {
      return stories.filter(isParanormalStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All Supernatural'
  ...
  return (
    <div className="min-h-screen bg-[#faf7ff] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#ddd6fe] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">Supernatural</h1>
            <p className="text-[11px] font-bold text-[#7c3aed]">Hidden powers, spirits, and unseen worlds</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#1e1b4b] via-[#6d28d9] to-[#22d3ee] p-5 text-white shadow-sm ring-1 ring-[#ddd6fe]">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h2 className="mt-1 text-[28px] font-black leading-8">Supernatural</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-white/85">
                Stories about spirits, hidden powers, magic energy, curses, mysterious abilities, and worlds beyond reality.
              </p>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-[12px] font-black active:scale-95 ${
                    active ? 'bg-white text-[#1e1b4b]' : 'bg-white/15 text-white ring-1 ring-white/25'
                  }`}
                >
                  {item.label}
                </button>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            ...
            <div className="text-[13px] font-black text-[#667085]">Opening supernatural...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest Supernatural" subtitle="Recently updated supernatural stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular Supernatural" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
            ...
            <section className="mt-7">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  ...
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No supernatural stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">Supernatural stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

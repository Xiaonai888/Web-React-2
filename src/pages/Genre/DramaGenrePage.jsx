...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'Drama',
...
function isDramaStory(item) {
  ...
  return values.some((value) => String(value || '').toLowerCase().includes('drama'))
}
...
function isEmotionalStory(story) {
  ...
  return ['drama', 'emotional', 'tears', 'cry', 'pain', 'heartbreak', 'family', 'conflict', 'secret', 'betrayal', 'love', 'hurt', 'life'].some((word) =>
    text.includes(word)
  )
}
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#fce7f3]">
        {story.cover ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[#be185d]">
            <i className="fa-solid fa-masks-theater text-[28px]" />
          </div>
        )}
      </div>
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A drama story filled with emotions, conflict, secrets, relationships, and life-changing moments.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#fce7f3] px-2.5 py-1 text-[#be185d]">Drama</span>
...
export default function DramaGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadDramaStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=Drama&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load drama stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isDramaStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load drama stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'emotional') {
      return stories.filter(isEmotionalStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All Drama'
  ...
  return (
    <div className="min-h-screen bg-[#fff7fb] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#fbcfe8] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">Drama</h1>
            <p className="text-[11px] font-bold text-[#be185d]">Emotion, conflict, and life stories</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#fce7f3] via-white to-[#ede9fe] p-5 shadow-sm ring-1 ring-[#fbcfe8]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#be185d] shadow-sm">
              <i className="fa-solid fa-masks-theater text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#be185d]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8 text-[#111827]">Drama</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-[#667085]">
                Emotional stories about conflict, relationships, secrets, family, love, pain, and life-changing decisions.
              </p>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-[12px] font-black active:scale-95 ${
                    active ? 'bg-[#111827] text-white' : 'bg-white text-[#be185d] ring-1 ring-[#fbcfe8]'
                  }`}
                >
                  {item.label}
                </button>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            <div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-4 border-[#fbcfe8] border-t-[#111827]" />
            <div className="text-[13px] font-black text-[#667085]">Opening drama...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest Drama" subtitle="Recently updated drama stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular Drama" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
            ...
            <section className="mt-7">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#fce7f3] text-[#be185d]">
                    <i className="fa-solid fa-masks-theater text-[24px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No drama stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">Drama stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

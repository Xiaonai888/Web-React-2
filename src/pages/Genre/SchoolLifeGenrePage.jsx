...
const filters = [
  { label: 'All School Life', value: 'all' },
...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'School Life',
...
function isSchoolLifeStory(item) {
  const values = [
    item.genre,
    item.category,
    item.main_genre,
  ...
  return values.some((value) => {
    ...
    return text.includes('school life') || text.includes('school-life') || text.includes('school')
  })
}
...
function isCampusStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['school', 'campus', 'classroom', 'student', 'teacher', 'academy', 'college', 'friendship', 'first love', 'youth', 'club', 'study'].some((word) =>
    text.includes(word)
  )
}
...
function StoryCard({ story, onOpen }) {
  return (
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#e0f2fe]">
        {story.cover ? (
          <img src={story.cover} alt={story.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
        ) : (
          <div className="flex h-full w-full items-center justify-center text-[#0284c7]">
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A school life story about youth, friendship, campus days, classroom moments, and first love.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#e0f2fe] px-2.5 py-1 text-[#0284c7]">School Life</span>
...
export default function SchoolLifeGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadSchoolLifeStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=School Life&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load school life stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isSchoolLifeStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load school life stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'campus') {
      return stories.filter(isCampusStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All School Life'
  ...
  return (
    <div className="min-h-screen bg-[#f5fbff] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#bae6fd] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-[#e0f2fe] text-[#111827] active:scale-95"
            aria-label="Back"
          >
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">School Life</h1>
            <p className="text-[11px] font-bold text-[#0284c7]">Youth, friendship, and campus days</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#e0f2fe] via-white to-[#fef3c7] p-5 shadow-sm ring-1 ring-[#bae6fd]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#0284c7] shadow-sm">
              <i className="fa-solid fa-book-open text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#0284c7]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8 text-[#111827]">School Life</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-[#667085]">
                Stories about campus days, classroom moments, friendship, first love, dreams, and growing up.
              </p>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-[12px] font-black active:scale-95 ${
                    active ? 'bg-[#111827] text-white' : 'bg-white text-[#0284c7] ring-1 ring-[#bae6fd]'
                  }`}
                >
                  {item.label}
                </button>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            <div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-4 border-[#bae6fd] border-t-[#111827]" />
            <div className="text-[13px] font-black text-[#667085]">Opening school life...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest School Life" subtitle="Recently updated school life stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular School Life" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
            ...
            <section className="mt-7">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#e0f2fe] text-[#0284c7]">
                    <i className="fa-solid fa-book-open text-[24px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No school life stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">School life stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

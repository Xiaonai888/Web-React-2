...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'Action',
...
function isActionStory(item) {
  ...
  return values.some((value) => String(value || '').toLowerCase().includes('action'))
}
...
function isIntenseStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['fight', 'battle', 'war', 'revenge', 'danger', 'power', 'hero', 'killer', 'blood', 'mission', 'survive', 'strong'].some((word) =>
    text.includes(word)
  )
}
...
function StoryCard({ story, onOpen }) {
  return (
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#111827]">
        {story.cover ? (
          <img src={story.cover} alt={story.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
        ) : (
          <div className="flex h-full w-full items-center justify-center text-[#ff6b35]">
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'An action story packed with danger, power, conflict, and intense moments.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#fff0e8] px-2.5 py-1 text-[#e54800]">Action</span>
...
export default function ActionGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadActionStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=Action&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load action stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isActionStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load action stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'intense') {
      return stories.filter(isIntenseStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All Action'
  ...
  return (
    <div className="min-h-screen bg-[#fff8f4] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#ffe0d1] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-[#fff0e8] text-[#111827] active:scale-95"
            aria-label="Back"
          >
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">Action</h1>
            <p className="text-[11px] font-bold text-[#e54800]">Power, danger, and intense battles</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#111827] via-[#7c2d12] to-[#ff6b35] p-5 text-white shadow-sm ring-1 ring-[#ffe0d1]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#e54800] shadow-sm">
              <i className="fa-solid fa-bolt text-[28px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#ffd7c2]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8">Action</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-white/85">
                Fast-paced stories filled with danger, conflict, battles, missions, power, and survival.
              </p>
            </div>
          </div>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            <div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-4 border-[#ffe0d1] border-t-[#111827]" />
            <div className="text-[13px] font-black text-[#667085]">Opening action...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest Action" subtitle="Recently updated action stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular Action" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
            ...
            <section className="mt-7">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#fff0e8] text-[#e54800]">
                    <i className="fa-solid fa-bolt text-[22px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No action stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">Action stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

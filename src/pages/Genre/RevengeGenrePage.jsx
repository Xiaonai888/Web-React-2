...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'Revenge',
...
function isRevengeStory(item) {
  ...
  return values.some((value) => String(value || '').toLowerCase().includes('revenge'))
}
...
function isPaybackStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['revenge', 'payback', 'betrayal', 'betrayed', 'justice', 'comeback', 'punish', 'enemy', 'vengeance', 'cold', 'return', 'destroy', 'karma'].some((word) =>
    text.includes(word)
  )
}
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#fee2e2]">
        {story.cover ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[#b91c1c]">
            <i className="fa-solid fa-scale-balanced text-[28px]" />
          </div>
        )}
      </div>
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A revenge story about betrayal, payback, justice, cold comeback, and power returning stronger.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#fee2e2] px-2.5 py-1 text-[#b91c1c]">Revenge</span>
...
export default function RevengeGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadRevengeStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=Revenge&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load revenge stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isRevengeStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load revenge stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'payback') {
      return stories.filter(isPaybackStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All Revenge'
  ...
  return (
    <div className="min-h-screen bg-[#fff7f7] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#fecaca] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">Revenge</h1>
            <p className="text-[11px] font-bold text-[#b91c1c]">Betrayal, payback, and cold justice</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#050505] via-[#7f1d1d] to-[#d1d5db] p-5 text-white shadow-sm ring-1 ring-[#fecaca]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#b91c1c] shadow-sm">
              <i className="fa-solid fa-scale-balanced text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#fecaca]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8">Revenge</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-white/85">
                Stories about betrayal, payback, cold comeback, justice, enemies, karma, and rising stronger after being broken.
              </p>
            </div>
          </div>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            ...
            <div className="text-[13px] font-black text-[#667085]">Opening revenge...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest Revenge" subtitle="Recently updated revenge stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular Revenge" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#fee2e2] text-[#b91c1c]">
                    <i className="fa-solid fa-scale-balanced text-[24px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No revenge stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">Revenge stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'BL',
...
function isBLStory(item) {
  ...
  return values.some((value) => {
    ...
    return text === 'bl' || text.includes('boys love') || text.includes('boy love')
  })
}
...
function isRomanceStory(story) {
  ...
  return ['bl', 'boys love', 'boy love', 'romance', 'love', 'heart', 'relationship', 'couple', 'bond', 'gentle', 'feelings'].some((word) =>
    text.includes(word)
  )
}
    ...
    >
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A BL story about soft romance, emotional bonds, gentle tension, modern love, and meaningful relationships.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#dbeafe] px-2.5 py-1 text-[#2563eb]">BL</span>
...
export default function BLGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadBLStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=BL&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load BL stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isBLStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load BL stories')
        }
      } finally {
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All BL'
  ...
  return (
    <div className="min-h-screen bg-[#f7fbff] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#bfdbfe] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">BL</h1>
            <p className="text-[11px] font-bold text-[#2563eb]">Soft romance, emotional bond, and modern love</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#2563eb] via-[#dbeafe] to-[#ddd6fe] p-5 shadow-sm ring-1 ring-[#bfdbfe]">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h2 className="mt-1 text-[28px] font-black leading-8 text-[#111827]">BL</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-[#667085]">
                Stories about boys love, soft romance, emotional connection, gentle tension, trust, and meaningful relationships.
              </p>
            </div>
          </div>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            ...
            <div className="text-[13px] font-black text-[#667085]">Opening BL...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest BL" subtitle="Recently updated BL stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular BL" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
            ...
            <section className="mt-7">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  ...
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No BL stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">BL stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

...
const filters = [
  { label: 'All Cold Male Lead', value: 'all' },
...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'Cold Male Lead',
...
function isColdMaleLeadStory(item) {
  const values = [
    item.genre,
    item.category,
    item.main_genre,
  ...
  return values.some((value) => {
    ...
    return text.includes('cold male lead') || text.includes('cold-male-lead') || text.includes('cold male')
  })
}
...
function isIcyStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['cold male lead', 'cold male', 'cold', 'icy', 'distant', 'emotionless', 'silent', 'powerful man', 'dark romance', 'heartless', 'stoic'].some((word) =>
    text.includes(word)
  )
}
...
function StoryCard({ story, onOpen }) {
  return (
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#dbeafe]">
        {story.cover ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[#2563eb]">
            <i className="fa-solid fa-snowflake text-[28px]" />
          </div>
        )}
      </div>
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A cold male lead story about distant love, icy emotion, powerful men, dark romance, and slow warmth.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#dbeafe] px-2.5 py-1 text-[#2563eb]">Cold Male Lead</span>
...
export default function ColdMaleLeadGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadColdMaleLeadStories() {
      setLoading(true)
      setMessage('')

      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=Cold%20Male%20Lead&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load cold male lead stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isColdMaleLeadStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load cold male lead stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'icy') {
      return stories.filter(isIcyStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All Cold Male Lead'
  ...
  return (
    <div className="min-h-screen bg-[#f7fbff] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#bfdbfe] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-[#dbeafe] text-[#111827] active:scale-95"
            aria-label="Back"
          >
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">Cold Male Lead</h1>
            <p className="text-[11px] font-bold text-[#2563eb]">Icy emotion, distant love, and dark romance</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#0f172a] via-[#1e3a8a] to-[#cbd5e1] p-5 text-white shadow-sm ring-1 ring-[#bfdbfe]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#2563eb] shadow-sm">
              <i className="fa-solid fa-snowflake text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#bfdbfe]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8">Cold Male Lead</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-white/85">
                Stories about cold personalities, distant love, powerful men, icy romance, slow emotion, and hearts that slowly soften.
              </p>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-[12px] font-black active:scale-95 ${
                    active ? 'bg-white text-[#0f172a]' : 'bg-white/15 text-white ring-1 ring-white/25'
                  }`}
                >
                  {item.label}
                </button>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            <div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-4 border-[#bfdbfe] border-t-[#111827]" />
            <div className="text-[13px] font-black text-[#667085]">Opening cold male lead...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest Cold Male Lead" subtitle="Recently updated cold male lead stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular Cold Male Lead" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#dbeafe] text-[#2563eb]">
                    <i className="fa-solid fa-snowflake text-[24px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No cold male lead stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">Cold male lead stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

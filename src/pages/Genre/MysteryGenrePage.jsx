...
function normalizeStory(item) {
  return {
    ...
    genre: item.genre || item.category || item.main_genre || 'Mystery',
...
function isMysteryStory(item) {
  ...
  return values.some((value) => String(value || '').toLowerCase().includes('mystery'))
}
...
function isDetectiveStory(story) {
  const text = `${story.title} ${story.description} ${story.status}`.toLowerCase()

  return ['detective', 'clue', 'case', 'secret', 'truth', 'murder', 'crime', 'investigate', 'hidden', 'suspect', 'evidence', 'mystery'].some((word) =>
    text.includes(word)
  )
}
...
function StoryCard({ story, onOpen }) {
  return (
    ...
    >
      <div className="h-[116px] w-[86px] shrink-0 overflow-hidden rounded-[16px] bg-[#eef2ff]">
        {story.cover ? (
          <img src={story.cover} alt={story.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
        ) : (
          <div className="flex h-full w-full items-center justify-center text-[#4f46e5]">
            <i className="fa-solid fa-user-secret text-[28px]" />
          </div>
        )}
      </div>
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <p className="mt-1 line-clamp-2 text-[12px] font-semibold leading-5 text-[#8d94a1]">
          {story.description || 'A mystery story filled with secrets, clues, hidden truths, suspense, and investigation.'}
        </p>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10.5px] font-black text-[#98a2b3]">
          <span className="rounded-full bg-[#eef2ff] px-2.5 py-1 text-[#4f46e5]">Mystery</span>
...
export default function MysteryGenrePage() {
  ...
  useEffect(() => {
    ...
    async function loadMysteryStories() {
      ...
      try {
        const response = await fetch(`${API_URL}/api/public/stories?genre=Mystery&limit=80`)
        const data = await response.json().catch(() => ({}))

        if (!response.ok || data.ok === false) {
          throw new Error(data.message || 'Failed to load mystery stories')
        }

        const rawStories = data.stories || data.items || data.results || []
        const filtered = rawStories.filter(isMysteryStory).map(normalizeStory)

        if (!ignore) setStories(filtered.length ? filtered : rawStories.map(normalizeStory))
      } catch (error) {
        if (!ignore) {
          setStories([])
          setMessage(error.message === 'Failed to fetch' ? 'Cannot connect to server.' : error.message || 'Failed to load mystery stories')
        }
      } finally {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (activeFilter === 'detective') {
      return stories.filter(isDetectiveStory)
    }
  ...
  const activeFilterLabel = filters.find((item) => item.value === activeFilter)?.label || 'All Mystery'
  ...
  return (
    <div className="min-h-screen bg-[#f8f7ff] pb-[110px]">
      <header className="sticky top-0 z-40 border-b border-[#ddd6fe] bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="flex h-10 w-10 items-center justify-center rounded-full bg-[#eef2ff] text-[#111827] active:scale-95"
            aria-label="Back"
          >
          ...
          <div className="min-w-0 text-center">
            <h1 className="text-[17px] font-black text-[#111827]">Mystery</h1>
            <p className="text-[11px] font-bold text-[#4f46e5]">Secrets, clues, and hidden truths</p>
          </div>
      ...
      <main className="mx-auto max-w-5xl px-4 pt-4">
        <section className="overflow-hidden rounded-[30px] bg-gradient-to-br from-[#111827] via-[#312e81] to-[#7c3aed] p-5 text-white shadow-sm ring-1 ring-[#ddd6fe]">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-[24px] bg-white text-[#4f46e5] shadow-sm">
              <i className="fa-solid fa-user-secret text-[30px]" />
            </div>

            <div className="min-w-0 flex-1">
              <div className="text-[12px] font-black uppercase tracking-[0.12em] text-[#c4b5fd]">Genre</div>
              <h2 className="mt-1 text-[28px] font-black leading-8">Mystery</h2>
              <p className="mt-2 max-w-[520px] text-[13px] font-semibold leading-6 text-white/85">
                Stories full of clues, secrets, hidden motives, suspicious characters, and truths waiting to be uncovered.
              </p>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-[12px] font-black active:scale-95 ${
                    active ? 'bg-white text-[#312e81]' : 'bg-white/15 text-white ring-1 ring-white/25'
                  }`}
                >
                  {item.label}
                </button>
        ...
        {loading ? (
          <section className="mt-5 rounded-[26px] bg-white p-7 text-center shadow-sm ring-1 ring-black/5">
            ...
            <div className="text-[13px] font-black text-[#667085]">Opening mystery...</div>
          </section>
        ) : null}
        ...
        {!loading && !message ? (
          <>
            <section className="mt-6">
              <SectionTitle title="Latest Mystery" subtitle="Recently updated mystery stories" />
            ...
            <section className="mt-7">
              <SectionTitle title="Popular Mystery" subtitle="Stories readers are watching now" />
              <div className="grid gap-3 sm:grid-cols-2">
              ...
              {filteredStories.length ? (
                ...
                <div className="rounded-[24px] bg-white p-8 text-center shadow-sm ring-1 ring-black/5">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#eef2ff] text-[#4f46e5]">
                    <i className="fa-solid fa-user-secret text-[24px]" />
                  </div>
                  <h3 className="mt-4 text-[17px] font-black text-[#111827]">No mystery stories yet</h3>
                  <p className="mt-2 text-[13px] font-semibold text-[#98a2b3]">Mystery stories will appear here when available.</p>
                </div>
              )}
            </section>
          </>

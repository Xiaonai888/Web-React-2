  ...
  return <section aria-label="Writing Studio" className="sd-writing-layout">
    ...
    <div className="sd-writing-main">
      <div className="sd-card sd-editor-card">
        ...
        <div className="mb-3 grid grid-cols-[minmax(0,1fr)_90px] gap-2"><label className="min-w-0 text-xs">Font<select className="sd-field mt-1 w-full" value={settings.font || 'Noto Serif Khmer'} disabled={!onChangeSettings} onChange={event => onChangeSettings?.({ font: event.target.value })}>{['Noto Serif Khmer', 'Noto Sans Khmer', 'Battambang', 'Georgia', 'Arial'].map(font => <option key={font} value={font}>{font}</option>)}</select></label><label className="min-w-0 text-xs">Size<select className="sd-field mt-1 w-full" value={settings.fontSize || 13} disabled={!onChangeSettings} onChange={event => onChangeSettings?.({ fontSize: Number(event.target.value) })}>{[10, 11, 12, 13, 14, 15, 16, 18, 20, 22, 24].map(size => <option key={size} value={size}>{size} pt</option>)}</select></label></div>
        ...
        <div key={`${book.id}-${chapter.id}`} ref={editorRef} data-chapter-id={chapter.id} className="sd-writing-area" contentEditable={typeof onChangeHTML === 'function'} suppressContentEditableWarning onInput={emitChange} onBlur={() => onEditorBlur?.(book.id)} onPaste={pastePlain} data-placeholder="Start writing your chapter…" role="textbox" aria-label="Chapter text editor" aria-multiline="true" spellCheck style={{ fontFamily: `"${settings.font || 'Noto Serif Khmer'}",serif`, fontSize: `${(Number(settings.fontSize) || 13) + 2}px`, lineHeight: settings.lineSpacing || 1.65, textAlign: settings.alignment || 'left', textIndent: `${firstLineIndent}mm`, '--sd-paragraph-spacing': `${paragraphSpacing}pt` }} />

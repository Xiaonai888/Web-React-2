...
export default function ReaderDiscoverPeoplePanel({
  open,
  profileUsername,
  onFollowed,
}) {
    ...
    <>
      ...
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={() =>
            navigate(`/profile/${encodeURIComponent(profileUsername)}/following`)
          }
            ...
            return (
              ...
              >
                <button
                  type="button"
                  onClick={() =>
                    navigate(`/profile/${encodeURIComponent(user.username)}/followers`)
                  }
                  className="block w-full text-center active:opacity-70"
                >
                  <ReaderAvatar user={user} />
                  <div className="mt-3 line-clamp-1 text-[14px] font-semibold text-[#111827]">
                    {user.name || user.username}
                  </div>
                  <div className="mt-0.5 line-clamp-1 text-[11px] font-normal text-[#6b7280]">
                    @{user.username}
                  </div>
                </button>

...
const DISCOVER_YOU_MIGHT_LIKE_CACHE_MAX_AGE_MS = 6 * 60 * 60 * 1000
...
export default function DiscoverYouMightLikeSection() {
  ...
  async function loadStories() {
    const cacheKey = getHomeCacheKey({
      ...
      params: {
        discover_section: 'you-might-like',
        sort: 'popular',
        limit: 10,
        schema: 1,
      },
    ...
    const cached = await loadHomeCache(cacheKey, {
      maxAgeMs: DISCOVER_YOU_MIGHT_LIKE_CACHE_MAX_AGE_MS,
      allowExpired: true,
    })
    ...
    try {
      ...
      const response = await fetch(
        addStoryLanguageParam(
          `${API_BASE_URL}/api/public/stories?limit=10&sort=popular`
        ),
        {

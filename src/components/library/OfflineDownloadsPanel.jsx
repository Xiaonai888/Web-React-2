...
export default function OfflineDownloadsPanel() {
  ...
  async function refresh() {
    ...
    try {
      ...
      if (mountedRef.current) setItems(records.filter(Boolean))
    } catch (reason) {
  ...
  return <section className="space-y-4">
    {selected ? <>
      ...
      {!busy && !groups.length ? <p className="rounded-xl border border-[var(--shadow-border)] p-5 text-sm text-[var(--shadow-text-secondary)]">{t('offlineDownloadsPanel.empty')}</p> : null}
    </>}
